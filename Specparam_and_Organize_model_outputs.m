...
%% this part of the code uses brainstorm to read in the time series data, compute the power spectrum, and fooof it
% % % % % % % % % % % % % % % % % % % % % % % % %
...
file2fooof= '/Users/jason/Downloads/TutorialOmega/data/Simulations/TriggerTest_SF031_20250204_01_bandwidthExtraWide/data_block001.mat';
timeseries= load('~/Documents/McGill/simulations4Giu/Template_timeseries.mat');

timeseries.Time=  0:0.0020:29.9980;
cd '/Users/jason/Documents/McGill/simulations4Giu/data/bandEPHYS/'
...
% % % % % % % % % % % % % % % % % % % % % % % % % %
cd('/Users/jason/Downloads/TutorialOmega/data/Simulations/TriggerTest_SF031_20250204_01_exponSmallWin/');
...
% adjust the list of parameters used to simulate the time series so that
% the output file is correctly labelled
...
% bandwidth = repelem([1,1.5,2,2.5,3,3.5,4,4.5,5], 500); 

PSD= zeros([numpar, 501]);
BIC= cell([numpar,1]);
mse_BIC= zeros([numpar,1]);
rsq_BIC=zeros([numpar,1]);
BF_model= ones(numpar, nchan);

% loop through all Fooofed spectra
count=1;
for i=1:numpar
...
for i=1:numpar
    ...
    fmse_BIC(i,:)=  squeeze(log10(PSD(i,3:101 )))-log10(BIC{i,1}.data(1).fooofed_spectrum);
...
meanPSD= zeros(10,99);
params_uni= unique(params);
for i=1:10
    meanPSD(i,:) = mean(log10(PSD(params== params_uni(i),3:101)));
...
figure
clear g
g(1,1)=gramm('x',repmat(PsdMat.Freqs(3:101),[10,1]),'y', meanPSD, 'color',categorical(params_uni));
%smooth plot 
g(1,1).geom_line();
g(1,1).set_title('FOOOFed spectrum');
g(1,1).set_color_options('map',colourmap(1:20:247,:));
...
meanPSD= zeros(10,99);
for i=1:10
    ranindex= 500*(i-1)+randi(500);
    meanPSD(i,:) = log(PSD(ranindex,3:101));

end

figure
clear g
g=gramm('x',repmat(log(PsdMat.Freqs(3:101)),[4,1]),'y', meanPSD([1,4,7,10],:), 'color',categorical(params_uni([1,4,7,10])));
%smooth plot 
g.geom_line();
g.set_title('FOOOFed spectrum');
g.set_color_options('map',colourmap(1:50:247,:));
%These functions can be called on arrays of gramm objects
g.axe_property('xticks',exp([1,5,10,15,20,25,30,25,40,25]));
g.set_names('x','Frequency (Hz)','y','Log Power');
g.draw();
ax = findall(gcf, 'Type', 'axes');

set(ax, ...
    'XTick', log([1,10,20,30,40]), ...
    'XTickLabels', [1,10,20,30,40]);

figure
clear g

g(1,1)=gramm('x',repmat(PsdMat.Freqs(3:101),[nchan,1]),'y', squeeze(mean(fmse_BIC,1)), 'color', categorical(repelem(1:nchan, 99)));
%smooth plot 
g(1,1).geom_line();
...
for i=1:numpar
    ...
    temp1=BIC{i,1}.data;
    fpsd_BIC(i,:)=  temp1(1).fooofed_spectrum(1:99);
...
figure
clear g
g(1,1)=gramm('x',repmat(PsdMat.Freqs(3:101),[2,1]),'y', [mean(log10(PSD(:,3:101)),1); BIC_psd], 'color',{'PSD','FOOOF'});
%smooth plot 
g(1,1).geom_line();
g(1,1).set_title('FOOOFed spectrum');
...
% make the matrix Nsims by 6 max num peaks 
...
% aperiodic corrected 
corr_spec_task=log10(PSD(:,3:101))-log10(fitaper_BIC);
corrlinear_spec_task=PSD(:,3:101)-fitaper_BIC;

figure
clear g

g(1,1)=gramm('x',PsdMat.Freqs(3:101),'y', squeeze(mean(corr_spec_task,1)), 'color', categorical(repelem(1:nchan, 99)));
%smooth plot 
g(1,1).geom_line();
...
figure
clear g
g(1,1)=gramm('x',repmat(PsdMat.Freqs(3:101),[2,1]),'y', [log10(PSD(psdNo,3:101))-log10(fitaper_BIC(psdNo,:));log10(fitper_BIC(psdNo,:)) ], 'color',{'PSD','FOOOF'});
%smooth plot 
g(1,1).geom_line();
g(1,1).set_title('FOOOFed spectrum');
g(1,1).set_color_options('map','brewer2');
...
figure
clear g
g(1,1)=gramm('x',repmat(PsdMat.Freqs(3:101),[2,1]),'y', [log10(PSD(psdNo,3:101));log10(fitaper_BIC(psdNo,:)) ], 'color',{'PSD','FOOOF'});
%smooth plot 
g(1,1).geom_line();
g(1,1).set_title('FOOOFed spectrum');
g(1,1).set_color_options('map','brewer2');
...
writetable(output, './Simulations_expon_samllwin_output.csv')

writematrix(BIC_peak_cf, './Simulations_expon_samllwin_peaks_centerFreq.csv')
writematrix(BIC_peak_amplitude, './Simulations_expon_samllwin_peaks_amplitude.csv')
writematrix(BIC_peak_std, './Simulations_expon_samllwin_peaks_std.csv')

writematrix(fpsd_BIC, './Simulations_expon_samllwin_fitted_spectrum.csv')
writematrix(fitaper_BIC, './Simulations_expon_samllwin_fitted_aperiodicspectrum.csv')
writematrix(fitper_BIC, './Simulations_expon_samllwin_fitted_periodicspectrum.csv')
writematrix(log10(PSD(:,3:101)), './Simulations_expon_samllwin_PSD.csv')

writematrix(corrlinear_spec_task, 'Exponent_smallwin_Simulations_corrspec_linear.csv')


% compute the expected spectrum based on the parameters so that we can
% compute signed error instead of MSE
apert_1= log10(PSD(1:500,4)) - log10(PsdMat.Freqs(3:101).^0.6);
apert_2= log10(PSD(501:1000,4)) - log10(PsdMat.Freqs(3:101).^0.7);
apert_3= log10(PSD(1001:1500,4)) - log10(PsdMat.Freqs(3:101).^0.8);
apert_4= log10(PSD(1501:2000,4)) - log10(PsdMat.Freqs(3:101).^0.9);
apert_5= log10(PSD(2001:2500,4)) - log10(PsdMat.Freqs(3:101).^1.0);
apert_6= log10(PSD(2501:3000,4)) - log10(PsdMat.Freqs(3:101).^1.1);
apert_7= log10(PSD(3001:3500,4)) - log10(PsdMat.Freqs(3:101).^1.2);
apert_8= log10(PSD(3501:4000,4)) - log10(PsdMat.Freqs(3:101).^1.3);
apert_9= log10(PSD(4001:4500,4)) - log10(PsdMat.Freqs(3:101).^1.4);
apert_10= log10(PSD(4501:5000,4)) - log10(PsdMat.Freqs(3:101).^1.5);
...
writematrix(aper_abs_error, 'Exponent_smallwin_Simulations__aperiodic_error_signed.csv')
...
y_norm =0.7*exp((-(PsdMat.Freqs(4:151) -10).^2)/(2*(1.^2)));
peakdist_1= y_norm+y_norm2;

y_norm =0.7*exp((-(PsdMat.Freqs(4:151) -10).^2)/(2*(1.5.^2)));
peakdist_2= y_norm+y_norm2;

y_norm =0.7*exp((-(PsdMat.Freqs(4:151) -10).^2)/(2*(2.^2)));
peakdist_3= y_norm+y_norm2;

y_norm =0.7*exp((-(PsdMat.Freqs(4:151) -10).^2)/(2*(2.5.^2)));
peakdist_4= y_norm+y_norm2;

y_norm =0.7*exp((-(PsdMat.Freqs(4:151) -10).^2)/(2*(3.^2)));
peakdist_5= y_norm+y_norm2;

y_norm =0.7*exp((-(PsdMat.Freqs(4:151) -10).^2)/(2*(3.5.^2)));
peakdist_6= y_norm+y_norm2;

y_norm =0.7*exp((-(PsdMat.Freqs(4:151) -10).^2)/(2*(4.^2)));
peakdist_7= y_norm+y_norm2;

y_norm =0.7*exp((-(PsdMat.Freqs(4:151) -10).^2)/(2*(4.5.^2)));
peakdist_8= y_norm+y_norm2;

y_norm =0.7*exp((-(PsdMat.Freqs(4:151) -10).^2)/(2*(5.^2)));
peakdist_9= y_norm+y_norm2;
...
writematrix(corrspec_abs_error, 'Bandwidth_Simulations_error_signed.csv')