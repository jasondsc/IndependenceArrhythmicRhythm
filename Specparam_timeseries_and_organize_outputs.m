...
addpath('/Users/jason/Documents/SickKids/abagen_analysis/natsortfiles')
file2fooof= '/Users/jason/Downloads/TutorialOmega/data/Simulations/TriggerTest_SF031_20250204_01_bandwidth/data_block001.mat';
timeseries= load('~/Documents/McGill/simulations4Giu/Template_timeseries.mat');
...
clear
close
cd('/Users/jason/Downloads/TutorialOmega/data/Simulations/TriggerTest_SF031_20250204_01_bandwidth/');
addpath('/Users/jason/Documents/github_repo/')
...
params= repelem([1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5], 500); 
% amplitude repelem([0.3, 0.5, 0.7, 0.9, 1.0,1.2,1.4, 1.6, 1.8, 2], 500); 
% expon repelem([0.6, 0.7, 0.8, 0.9, 1.0,1.1, 1.2, 1.3, 1.4, 1.5], 500); 
% freqs repelem([6,7,8,9,10,11,12,13,14,], 500);
...
% aperiodic corrected 
corr_spec_task=log10(PSD(:,4:151))-log10(fitaper_BIC);
corrlinear_spec_task=PSD(:,4:151)-fitaper_BIC;
...
figure
clear g
g(1,1)=gramm('x',repmat(PsdMat.Freqs(4:151),[3,1]),'y', [mean(log10(PSD(params == 5,4:151)),1); mean(log10(PSD(params == 3,4:151)),1); mean(log10(PSD(params == 1,4:151)),1) ], 'color',{'small','medium','large'});
%smooth plot 
g(1,1).geom_line();
g(1,1).set_title('FOOOFed spectrum');
g(1,1).set_color_options('map','brewer2');
...
cd('/Users/jason/Documents/McGill/simulations4Giu/')
output= table(params',expon_BIC, offset_BIC, mse_BIC, rsq_BIC, BF_model);
writetable(output, './Simulations_bandwidth_output.csv')

writematrix(BIC_peak_cf, './Simulations_bandwidth_peaks_centerFreq.csv')
writematrix(BIC_peak_amplitude, './Simulations_bandwidth_peaks_amplitude.csv')
writematrix(BIC_peak_std, './Simulations_bandwidth_peaks_std.csv')

writematrix(fpsd_BIC, './Simulations_bandwidth_fitted_spectrum.csv')
writematrix(fitaper_BIC, './Simulations_bandwidth_fitted_aperiodicspectrum.csv')
writematrix(fitper_BIC, './Simulations_bandwidth_fitted_periodicspectrum.csv')
writematrix(log10(PSD(:,4:151)), './Simulations_bandwidth_PSD.csv')


writematrix(corr_spec_task, 'Bandwidth_Simulations_corrspec.csv')
writematrix(corrlinear_spec_task, 'Bandwidth_Simulations_corrspec_linear.csv')

%% Now let us compute rhythmic estimated error 

% exponent simulations
y_norm2= 0.4*exp((-(PsdMat.Freqs(4:151) -19).^2)/(2*(5.^2)));
y_norm =0.7*exp((-(PsdMat.Freqs(4:151) -10).^2)/(2*(2.^2)));
peakdist_1= y_norm+y_norm2;
